window.onload = function () {
	...
	document.body.onmousemove = function (e) {
		...
		if (Math.random() < 0.6) shape.x = e.pageX;
		if (Math.random() < 0.6) shape.y = e.pageY;
	...
	function getRandomShape() {
		...
		return potentialShapes[
			Math.round(Math.random() * (potentialShapes.length - 1))
		];
	}
	...
	function Circle(x, y, radius, rotation, startAngle, endAngle, fill) {
		...
		rotation =
			typeof rotation !== 'undefined' ? rotation : (45 * Math.PI) / 180;
